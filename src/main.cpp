...
#if defined(NDEBUG)
# error "googcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x75fe8fb9786117721c5f1ad3397d1d57a0c8f97f553993a4952d34ce2b79c927");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // googcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "googcoin Signed Message:\n";
...
{
    // googcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // googcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // googcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // googcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // googcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // googcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe0bb384e18446164c9a83d7bdcd1a30b4ffcc5d09305fceeea304504ce270495");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0475185c361155e17f7382007d2e29abf0ab54a716da2edc247e5d21adf76c864259884b1a2207ef778a7e073c057f6e0130dd29a78ded22f3957dfd4742adc92a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1545227616;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086582730;

        if (fTestNet)
        {
            block.nTime    = 1545227543;
            block.nNonce   = 386837913;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7fb3e5d89eceab9e366fae8352ada43b1a69e5f715ad0c0e8a709627e86a9e59"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // googcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("googcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("googcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static googcoinMiner(CWallet *pwallet)
{
    printf("googcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("googcoin-miner");
    ...
    try { loop {
        ...
        printf("Running googcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("googcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&googcoinMiner, pwallet));
}
